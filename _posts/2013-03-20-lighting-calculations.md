...
<table>
    <tr>
        <td rowspan="2">расчётная высота подвеса светильника, \\(h_{расч}\\), м</td>
        <td rowspan="2">площадь s, м<sup>2</sup></td>
        <td colspan="4">удельная мощность (Вт/м<sup>2</sup>) для седьмой группы светильников с лампами типов (\\(\ro_п\\) = 70%, \\(\ro_c\\) = 50%</td>
    </tr>
    <tr>
        <td>ЛБ40, 65</td>
